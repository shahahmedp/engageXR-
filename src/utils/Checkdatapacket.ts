...
export const companyCheck = [
  check('email', 'invalid email').isEmail().isLength({ min: 3, max: 30 }),
  check('name', 'Name length should be 10 to 20 characters').isLength({
    max: 20,
  }),
  check('website', 'website length should be 10 to 20 characters').isLength({
    min: 4,
    max: 20,
  }),
...
export const employeeCheck = [
  check('email', 'invalid email').isEmail().isLength({ max: 30 }),
  check(
    'firstName',
    'First Name length should be 10 to 20 characters'
  ).isLength({ max: 20 }),
